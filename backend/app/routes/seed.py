...
@router.post("/seed")
async def seed_mock_data(db: Session = Depends(get_db)):
    """Seed database with mock data for AAPL, MSFT, GOOGL, AMZN"""
    
    try:
        # Clear existing data for these companies
        ...
        for company in companies:
            ...
            for idx, qa in enumerate(company['qa_items']):
                qa_item = QAItem(
                    ...
                    topic=qa.get('topic'),
                    deflection_score=qa.get('deflection_score')
                )
                db.add(qa_item)
